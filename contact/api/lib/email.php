...
class Email
{
    ...
    private function _smtp_pop_auth()
    {
        ...
        $this->_smtp_pop_auth_recv($con, 'open');
        if ($protocol == 'tls') {
            $this->_smtp_pop_auth_send($con, 'stls');
        ...
        $this->_smtp_pop_auth_send($con, 'user', $this->config('smtp_pop_user'));
        $this->_smtp_pop_auth_send($con, 'pass', $this->config('smtp_pop_pass'));
        $this->_smtp_pop_auth_send($con, 'quit');
        fclose($con);
        // set cache time
        if (! empty($cache_path)) {
    ...
    /**
     * Authenticate pop before smtp Connect Receiver
     * @param resource &$con : pop connection
     * @parma string   $cmd  : command
     * @return void
     */
    private function _smtp_pop_auth_recv(&$con, $cmd) {
    ...
    /**
     * Authenticate pop before smtp Connect Sender
     * @param resource &$con : pop connection
     * @parma string   $cmd  : command
     * @param string   $val  : value
     * @return void
     */
    private function _smtp_pop_auth_send(&$con, $cmd, $val = '') {
        ...
        $this->_smtp_pop_auth_recv($con, $cmd);
    }
    ...
    {
        return implode('-', array_map('ucwords', explode('-', $name))) . ': ' . $this->_data_directive($name, $data);
    }
